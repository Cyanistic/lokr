...
const FilePreviewModal: React.FC<FilePreviewModalProps> = ({ isOpen, onClose, file }) => {
  ...
  const renderContent = () => {
    ...
    if (file.type.startsWith('audio/') || fileExtension === 'mp3' || fileExtension === 'wav' || fileExtension === 'm4a') {
      return renderAudio();
    }

    if (file.type.startsWith('video/') || fileExtension === 'mp4' || fileExtension === 'mov' || fileExtension === 'webm') {
      return renderVideo();
    }

    if (file.type.startsWith('image/') || ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'webp', 'svg'].includes(fileExtension || '')) {
      return renderImage();
    }
